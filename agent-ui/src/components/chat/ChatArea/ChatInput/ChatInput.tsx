  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      if (file.size > MAX_FILE_SIZE) {
        toast.error(
          `Файл "${file.name}" слишком большой (макс. ${formatFileSize(MAX_FILE_SIZE)})`
        )
        continue
      }
      if (
    ...
    <div className="mx-auto mb-1 flex w-full max-w-2xl flex-col gap-2 font-geist">
      ...
      {hasFiles && (
        <div className="flex flex-wrap gap-2 px-1">
          {attachedFiles.map(({ file, id }) => (
            <div
              key={id}
              className="group flex items-center gap-1.5 rounded-lg border border-primary/10 bg-primaryAccent px-2.5 py-1.5 text-xs text-secondary"
            >
              <Icon
      ...
      <div className="flex items-end justify-center gap-x-2">
        ...
        <Button
          ...
          size="icon"
          variant="ghost"
          className="h-10 w-10 rounded-xl text-muted hover:text-primary"
        ...
        <TextArea
          ...
          className="border-primary/12 w-full rounded-xl border bg-primaryAccent px-4 py-3 text-sm text-white placeholder:text-muted focus:border-primary/25 focus:outline-none"
        ...
        <Button
          ...
          disabled={!canSubmit}
          size="icon"
          className="h-10 w-10 rounded-xl bg-brand text-white hover:bg-brand/80 disabled:opacity-50"
        >
