  ...
  return (
    <div className="flex items-start gap-4 pt-4 text-start max-md:break-words">
      ...
      <div className="flex flex-col gap-2">
        {hasFiles && (
          <div className="flex flex-wrap gap-1.5">
            {message.attached_files!.map(
              (file: AttachedFileInfo, i: number) => (
                <div
                  key={`${file.name}-${i}`}
                  className="inline-flex items-center gap-1.5 rounded-md border border-primary/10 bg-primaryAccent px-2 py-1 text-xs text-muted"
                >
        ...
        <div className="font-geist text-sm leading-relaxed text-secondary">
          {message.content}
        </div>
      </div>
    </div>
