  ...
  const handleDeleteSession = async () => {
    ...
    try {
      ...
      if (response?.ok && sessionsData) {
        ...
        toast.error(
          `Не удалось удалить сессию: ${response?.statusText || 'Unknown error'} ${errorMsg || ''}`
        )
      }
    } catch (error) {
      toast.error(
        `Не удалось удалить сессию: ${error instanceof Error ? error.message : String(error)}`
      )
    } finally {
    ...
    <>
      <div
        className={cn(
          ...
          isSelected
            ? 'border-l-2 border-brand bg-accent'
